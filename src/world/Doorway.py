...
class Doorway:
    def __init__(self, direction, open, room):
        ...
        if direction == 'left':
            self.x = 0
            self.y = (MAP_HEIGHT / 2 * TILE_SIZE) - TILE_SIZE
            self.height = 96
            self.width = 48
        elif direction == 'right':
            self.x = (MAP_WIDTH * TILE_SIZE) - TILE_SIZE
            self.y = 0
            self.height = MAP_WIDTH*TILE_SIZE
            self.width = 48
        elif direction == 'top':
            self.x = (MAP_WIDTH / 2 * TILE_SIZE) - TILE_SIZE
            self.y = 0
            self.height = 48
            self.width = 96
        else:
            self.x = (MAP_WIDTH / 2 * TILE_SIZE) - TILE_SIZE
            self.y = (MAP_HEIGHT * TILE_SIZE) - TILE_SIZE
            self.height = 48
            self.width = 96
    ...
    def render(self, screen, offset_x, offset_y):
        ...
        if self.direction == 'left':
            ...
            if self.open:
                ...
                index = 0
            screen.blit(door_image[index], (self.x, self.y + (MAP_HEIGHT/2)*TILE_SIZE))
