...
class EntityWalkState(BaseState):
    ...
    def ProcessAI(self, params, bossCoords, dt):
        ...
        #print(self.calculate_distance(bossCoords, self.entity.x, self.entity.y))

        if bossCoords is None:
            pass
        elif self.calculate_distance(bossCoords, self.entity.x, self.entity.y) > 100:
            #now = datetime.now()
            #current_time = now.strftime("%H:%M:%S")
            #print("Current Time =", current_time)
            direction = self.return_direction(bossCoords, self.entity.x, self.entity.y)
